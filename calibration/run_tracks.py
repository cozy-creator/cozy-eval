"""Regenerate ``track-stability.json`` — the evidence that fixes the @8 floor.

Scores the LABELED H3 corpus through the shipped library path
(:func:`cozy_eval.metrics.tracks.track_stats`) and reports the three-way
separation the family had to clear before it shipped:
...
def pairs_table(scores: dict[str, dict], pairs: list[tuple[str, str, str]]) -> list[dict]:
    ...
    for label, ref, cand in pairs:
        ...
        if a["track_survival"] < T.TRACKABILITY_FLOOR or a["track_stability"] <= 1e-9:
            ...
            row["ratio"] = raw
            row["verdict"] = "reject" if raw < T.STABILITY_RATIO_FLOOR else "pass"
        rows.append(row)
    return rows
...
def main() -> int:
    ...
    every = sorted({c for _, a, b in REJECTED + IDENTICAL + CONTROLS for c in (a, b)})
    ...
    doc = {
        "library": "cozy-eval",
        "metric_set": "cozy-eval/metrics@8",
        "what": ("track-stability calibration: the owner-labeled three-way "
                 "separation that fixed STABILITY_RATIO_FLOOR"),
    ...
    print()
    for name in ("rejected", "identical", "controls"):
